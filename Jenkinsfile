  ...
  stage("View installed charts") {
    steps {
      script{
        ...
        echo "\033[0;35m \$(/usr/local/bin/helm ls --deployed $chart_name --output json | jq -r .Releases[] | sed 's/{//g;s/}//g;s/"//g;s/,//g;s/^[ \t]*//g')\033[0m"
        """
      }
    }
  }
  ...
  stage("Get status") {
    steps {
      script{
        ...
        echo "\033[0;35m \$(/usr/local/bin/helm ls --deployed $chart_name --namespace $environment --output json | jq -r .Releases[] | sed 's/{//g;s/}//g;s/"//g;s/,//g')\033[0m"
        """
        }
      }
    }           
